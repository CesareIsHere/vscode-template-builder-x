...
const yaml = __webpack_require__(3);
const path = __webpack_require__(28);
function activate(context) {
    let scratchTemplateDisposable = vscode.commands.registerCommand("extension.generateScratchTemplate", async (uri) => {
        try {
            var activeFolder;
            var outputPath;
    ...
    let defaultTemplateDisposable = vscode.commands.registerCommand("extension.generateDefaultTemplate", async (uri) => {
        try {
            var activeFolder;
            var outputPath;
            if (!uri) {
    ...
    let generateFolderDisposable = vscode.commands.registerCommand("extension.generateApiFolder", async (uri) => {
        try {
            var activeFolder;
            var outputPath;
            if (!uri) {
...
function generateFromYamlData(yamlData, currentPath) {
...
function getTemplatesFolderPath(context) {
